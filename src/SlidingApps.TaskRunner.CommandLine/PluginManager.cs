...
namespace SlidingApps.TaskRunner.CommandLine
{
    public static class PluginManager
    {
        ...
        private static List<Type> GetPluginTypes()
        {
            ...
            assemblyFiles.ForEach(a => {
                try
                {
                    types.AddRange(Assembly.LoadFrom(a).GetTypes());
                }
                catch { /* IGNORE */}
            });
            ...
            var plugins =
                assemblyFiles
                    .SelectMany(a => types.Where(t => typeof(IPlugin).IsAssignableFrom(t) && !t.IsInterface && !t.IsAbstract))
                    .Distinct();
