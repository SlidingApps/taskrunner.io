CREATE TABLE `Person_H` (
  ...
  `OrganizationID` CHAR(36) NOT NULL COMMENT '',
  ...
  INDEX `IDX_Person_H_OrganizationID` (`OrganizationID` ASC)  COMMENT '',
  INDEX `IDX_Person_H_StartDate-EndDate` (`StartDate`, `EndDate` ASC)  COMMENT '',
  FOREIGN KEY (`OrganizationID`) REFERENCES `Organization_H` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE);